...
class Model {
    ...
    func getGrade(of nickname: String) -> String{
        if let grade = dataController.getGrade(of: nickname){
            return grade
        }
    ...
    func saveGrade(of nickname: String, results: [Int]){
        var grade = 0
        
        for i in 0..<results.count{
            let questionAnswer = questions[i].answer.rawValue - 1
            let userAnswer = results[i]
        ...
        if let user = dataController.getUser(nickname: nickname){
            user.grade = "\(grade) / \(results.count)"
